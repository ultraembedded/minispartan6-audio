...
//-----------------------------------------------------------------
// _sd_erase: Erase specified number of blocks of SD card
//-----------------------------------------------------------------
static uint8_t _sd_erase(uint32_t start_block, uint32_t total_blocks)
{
    uint8_t response;

    response = _sd_send_command(CMD32_ERASE_WR_BLK_START, start_block);
    if(response != 0x00)
        return response;

    response = _sd_send_command(CMD33_ERASE_WR_BLK_END,(start_block + total_blocks - 1));
    if(response != 0x00)
        return response;

    response = _sd_send_command(CMD38_ERASE, 0);
    if(response != 0x00)
        return response;
